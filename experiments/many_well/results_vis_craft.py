...
def plot_manywell_results(cfg: DictConfig,
    ...
    samples_flow = torch.tensor(np.load(path_to_model))[:n_samples]
...
def run(cfg: DictConfig):
    ...
    titles = ["CRAFT (config 1)", "CRAFT (config 2)"]
    seed = 1
    ...
    plot_manywell_results(cfg, subfigs[0], path_to_model='craft_custom_config_samples.npy', plot_y_label=True)
    subfigs[0].suptitle(titles[0])

    plot_manywell_results(cfg, subfigs[1], path_to_model='craft_original_config_samples.npy', plot_y_label=False)
